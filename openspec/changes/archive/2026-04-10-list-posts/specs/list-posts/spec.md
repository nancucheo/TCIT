...
The `App.tsx` root component SHALL render `ErrorBoundary > Layout > PostList`. The Layout component SHALL display the title "TCIT Posts Manager" inside a Bootstrap Container. The ErrorBoundary SHALL catch render errors and display a fallback danger alert.
