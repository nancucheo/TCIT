---
description: Standards and best practices for technical documentation in the TCIT Posts Manager project, including documentation structure, update processes, and language rules.
globs:
alwaysApply: true
---
...
2. Identify which documentation files need updates based on the changes. Some clear examples:
   ...
   - For API changes: Update `api-spec.yml` and `docs/openapi.yaml`
   - For changes in libraries, database migrations, or anything that changes the installation process: Update `backend-standards.md` or `frontend-standards.md`
   - For infrastructure changes: Update `docs/docker-spec.md`, `docs/terraform-spec.md`, or `docs/cicd-spec.md`
...
- File format: `NNN-short-description.md` (e.g., `001-express-ddd-over-nestjs.md`)
- Include: Context, Decision, Consequences (positive and negative)
- Status: Proposed → Accepted → Superseded (if replaced)
- Always reference the ADR in related documentation
